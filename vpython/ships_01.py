...
class Stinger( VP_Sprite ):
    ...
    def get_geometry( self ):
        return [
            # Front fusilage
            cone( axis = vector( 0.0 , 0.0 , 8.0 ) , 
                  radius = 3.0 , 
                  color = shipGreen ),
            # Rear fusilage      
            cone( axis = vector(  0.0 ,  0.0 , -4.0 ) , 
                  radius = 3.0 , 
                  color = shipGreen ),
            # Top Fin
            triangle(
                v0 = vertex( pos = vec(  0.0 ,  0.0 ,  0.0 ) , color = shipOrange ),
                v1 = vertex( pos = vec(  0.0 ,  0.0 ,  8.0 ) , color = shipOrange ),
                v2 = vertex( pos = vec(  0.0 ,  6.0 , -4.0 ) , color = shipOrange ) , 
                color = vector( 106/255 , 196/255 , 124/255 ) 
            ),
            # Left Fin
            triangle(
                v0 = vertex( pos = vec(  0.0 ,  0.0 ,  0.0 ) , color = shipBlue ),
                v1 = vertex( pos = vec(  0.0 ,  0.0 ,  8.0 ) , color = shipBlue ),
                v2 = vertex( pos = vec( -6.0 , -6.0 , -4.0 ) , color = shipBlue ) , 
                color = vector( 106/255 , 196/255 , 124/255 ) 
            ),
            # Right Fin
            triangle(
